...
def bubble_sort(department_data):
    n = len(department_data)
    for i in range(n):
        for j in range(0, n-i-1):
            employee1 = department_data[j]
            employee2 = department_data[j+1]
            ...
            if should_swap is True:
                department_data[j], department_data[j+1] = department_data[j+1], department_data[j]
...
for department_name, _ in department_totals:
    # Ordenar los datos del departamento con Bubble Sort
    bubble_sort(departments[department_name])
...
all_data.to_excel(f"C:\\Users\\Cesar\\PycharmProjects\\Datos_Parcial_III\\DatosBubbleSort.xlsx", index=False)
ubicacion= f"C:\\Users\\Cesar\\PycharmProjects\\Datos_Parcial_III\\DatosBubbleSort.xlsx"
fin = time.time()
print("El archivo se ha ordenado con exito y se ha guardado en la ruta especificada", ubicacion)
print(f"El tiempo de ejecución del metodo Burbuja fue de {fin-inicio} segundos")
